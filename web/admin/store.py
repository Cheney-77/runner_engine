...
class AdminStore:
    def __init__(self, dsn: str, connect=None):
        ...
        self._schema_ready = bool(connect)
        self._schema_lock = threading.Lock()

        # Production path: once ADMIN_DB_URL is configured, initialize the
        # Admin-owned schema synchronously. DDL/permission errors must fail
        # startup instead of leaving a half-working Admin UI.
        if self.dsn and not self.connect_factory:
            self._ensure_schema()
    ...
    def _open_connection(self):
        ...
        if not self.dsn:
            raise StorageUnavailable(
                "Set ADMIN_DB_URL; console_ops is initialized automatically."
            )
        ...
        try:
            ...
            log.warning(
                "Console metadata connection failed (%s)",
                type(exc).__name__,
            )
            raise StorageUnavailable(
    ...
    def _ensure_schema(self):
        ...
        with self._schema_lock:
            ...
            try:
                ...
                log.exception("console_ops initialization failed")
                raise StorageUnavailable(
                    "console_ops initialization failed; check CREATE/USAGE "
                    "permissions for ADMIN_DB_URL."
                ) from exc
    ...
    def overview(self):
        if not self.dsn and not self.connect_factory:
            return {
                "status": "unconfigured",
                "message": (
                    "Configure ADMIN_DB_URL; console_ops will be initialized "
                    "automatically."
                ),
                "notes": [],
                "requests": [],
        ...
        with self.connection() as conn:
            ...
            audits = conn.execute(
                """SELECT id::text, actor, action, resource_type, resource_id,
                          occurred_at
                   FROM console_ops.audit_events
    ...
    def update_note(self, actor, uid, note, version):
        ...
        with self.connection() as conn:
            ...
            if not row:
                raise Conflict(
                    "Note changed or no longer exists. Refresh and try again."
                )
    ...
    def delete_note(self, actor, uid, version):
        ...
        with self.connection() as conn:
            row = conn.execute(
                """DELETE FROM console_ops.asset_notes
                   WHERE id=%s::uuid AND version=%s
