...
def make_app(settings: Settings, *, database_readers=None, sandbox_reader=None):
    app = FastAPI(
        title="Managed Python Exact PostgreSQL Observability",
        docs_url=None, redoc_url=None, openapi_url=None,
    )
    app.state.readers = database_readers or exact_readers(
        settings.db_urls, schemas=settings.db_schemas
    )
    ...
    async def health():
        return {"ok":True,"service":"exact-postgresql-observability"}
    ...
    async def database_api():
        ...
        return {"sampledAt":datetime.now(timezone.utc).isoformat(),
    ...
    async def overview():
        databases,sandbox=await asyncio.gather(database_api(),sandbox_api())
        return {"sampledAt":datetime.now(timezone.utc).isoformat(),
                "databases":databases["services"],"sandboxes":sandbox}

    app.mount("/observe/static",StaticFiles(directory=OLD_STATIC),
              name="original-observe-css-and-assets")
    return app
